...
# -- Stan modeling --
file <- file.path("stan_models\\WSLS.stan")
...
# -- Stan modeling --
file <- file.path("stan_models\\WSLS_multiagent.stan")
...
multi_agent_draws %>%
  ...
  ggplot(aes(x=value, y=name, fill=..y.., color="white")) +
...
ggsave("plots/multi_agent_staybias.jpg", width=7.29, height=4.5)
...
multi_agent_draws %>%
  ...
  annotate("text", label="Actual Parameter Value", x=WSLS_leavebias-0.17, y=10.3) +
  xlim(0, 1) +
...
ggsave("plots/multi_agent_leavebias.jpg", width=7.29, height=4.5)
...
# Stan model setup
file <- file.path("models\\WSLS_multiagent.stan")
multi_agent_stan <- cmdstan_model(file, cpp_options = list(stan__threads = TRUE))
...
multi_trial_draws %>%
  ...
  ggplot(aes(x=value, y=trials, fill=..y.., color="white")) +
  geom_density_ridges(rel_min_height = 0.01, scale=2.5) +
  scale_fill_gradient(low="deepskyblue4", high="deepskyblue3") +
  scale_color_manual(values = c("white")) + 
  geom_vline(xintercept=WSLS_staybias, size=1.1, color="firebrick3", alpha=0.7) +
  annotate("text", label="Actual Parameter Value", x=WSLS_staybias-0.17, y=7) +
  xlim(0, 1) +
...
ggsave("plots/multi_trial_staybias.jpg", width=7.29, height=4.5)
...
multi_trial_draws %>%
  ...
  scale_color_manual(values = c("white")) + 
  geom_vline(xintercept=WSLS_leavebias, size=1.1, color="firebrick3", alpha=0.7) +
  annotate("text", label="Actual Parameter Value", x=WSLS_leavebias-0.17, y=7) +
  xlim(0, 1) +
...
ggsave("plots/multi_trial_leavebias.jpg", width=7.29, height=4.5)
